...
class Level:
            ...
            for row_index, row in enumerate(layout):
                for col_index, col in enumerate(row):
                    if col != '-1':
                        ...
                        if style == 'entities':
                            if col == '394':
                                self.player = Player((x, y), [self.visible_sprites], self.obstacle_sprites, self.create_attack, self.destroy_attack, self.create_magic, self.destroy_magic)
                            else:
                                if col == '390':
                                    monster_name = 'bamboo'
                                elif col == '391':
    ...
    def damage_player(self, ammount, attack_type):
        if self.player.vulnerable:
            ...
            self.animation_player.create_particles(attack_type, pos, groups)
                    ...
                    for target_sprite in collision_sprites:
                        if target_sprite.sprite_type == 'grass':
                            ...
                            for leaf in range(randint(3, 6)):
                                self.animation_player.create_grass_particles(pos - offset, group)
                            target_sprite.kill()
                        else:
